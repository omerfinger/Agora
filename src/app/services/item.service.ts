...
export class ItemService {
  private items: Item[];
  private craigsListItems: Object[];
  ...
  getItemsByKind(kind: Kind) {
    ...
    var obs = this.http.get(URI).pipe(map(res => res.json()));
    obs.subscribe(res => {
      this.items = res.items.filter(item => item.kind == kind);
  ...
  getCraigslistItems() {
    let URI = `${this.serverApi}/craigslist`;
    var obs = this.http.get(URI).pipe(map(res => res.json()));
    obs.subscribe(res => {
      this.craigsListItems = res;
    });
    return obs;
  }
  ...
  searchItems(sname: string, skind: Kind, scategory: Category, stime: Date) {
    let URI = `${
      this.serverApi
    }/items/search/${sname}-${skind}-${scategory}-${stime}`;
    var obs = this.http.get(URI).pipe(map(res => res.json()));
    obs.subscribe(res => {
      this.items = res.items;
      this.itemsUpdate.next([...this.items]);
  ...
  createItem(newItem: Item) {
    ...
    this.http
      .post(URI, JSON.stringify(newItem), { headers: headers })
      .subscribe(
        data => {
